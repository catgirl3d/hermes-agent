...
export const zh: Translations = {
  ...
  sidebar: {
    ...
    row: {
      ...
      openInSplit: '在分屏中打开',
      cleanToolOutputs: '清理工具输出',
      cleanToolOutputsChoose: '选择要压缩的工具输出',
      cleanToolOutputsCodeWarning: '文件/代码压缩会丢失信息，可能降低回答质量或迫使模型重新读取文件。',
      cleanToolOutputsConfirm: '清理上下文',
      cleanToolOutputsDone: '上下文已清理',
      cleanToolOutputsFailed: '无法清理工具输出',
      cleanToolOutputsNoop: '没有足够大的旧工具输出可清理。',
      cleanToolOutputsKinds: {
        bounded_excerpt: '保留开头和结尾',
        file_structure: '有损提纲或开头与结尾',
        terminal_tail: '输出末尾；如有结构化错误也会保留',
        web_fields: '前5个结果字段'
      },
      cleanToolOutputsPreview: (resultCount, argumentCount, tokens) =>
        `将压缩 ${resultCount} 个旧结果和 ${argumentCount} 个调用载荷，预计节省 ${tokens} 个 token。`,
      cleanToolOutputsProtected: turns => `最近 ${turns} 轮对话将保持原样。`,
      cleanToolOutputsStale: '会话已更改。请检查更新后的预览并再次确认。',
      cleanToolOutputsToggle: name => `压缩 ${name} 输出`,
      cleanToolOutputsToolMeta: (resultCount, argumentCount, tokens, policy) =>
        `${resultCount} 个结果 + ${argumentCount} 个调用载荷 · 约 ${tokens} 个 token · ${policy}`,
      cleanToolOutputsTitle: '清理工具输出？',
      cleanToolOutputsUpdating: '正在更新预览…',
      cleanToolOutputsWarning: '这会重写当前会话历史并导致一次提示缓存未命中。此操作无法撤销。',
