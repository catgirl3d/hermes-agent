...
export const zhHant = defineLocale({
  ...
  sidebar: {
    ...
    row: {
      ...
      archive: '封存',
      newWindow: '新視窗',
      cleanToolOutputs: '清理工具輸出',
      cleanToolOutputsChoose: '選擇要壓縮的工具輸出',
      cleanToolOutputsCodeWarning: '檔案/程式碼壓縮會遺失資訊，可能降低回答品質或迫使模型重新讀取檔案。',
      cleanToolOutputsConfirm: '清理上下文',
      cleanToolOutputsDone: '上下文已清理',
      cleanToolOutputsFailed: '無法清理工具輸出',
      cleanToolOutputsNoop: '沒有足夠大的舊工具輸出可清理。',
      cleanToolOutputsKinds: {
        bounded_excerpt: '保留開頭與結尾',
        file_structure: '有損提綱或開頭與結尾',
        terminal_tail: '輸出末尾；如有結構化錯誤也會保留',
        web_fields: '前5個結果欄位'
      },
      cleanToolOutputsPreview: (resultCount, argumentCount, tokens) =>
        `將壓縮 ${resultCount} 個舊結果和 ${argumentCount} 個呼叫負載，預計節省 ${tokens} 個 token。`,
      cleanToolOutputsProtected: turns => `最近 ${turns} 輪對話將保持原樣。`,
      cleanToolOutputsStale: '工作階段已變更。請檢查更新後的預覽並再次確認。',
      cleanToolOutputsToggle: name => `壓縮 ${name} 輸出`,
      cleanToolOutputsToolMeta: (resultCount, argumentCount, tokens, policy) =>
        `${resultCount} 個結果 + ${argumentCount} 個呼叫負載 · 約 ${tokens} 個 token · ${policy}`,
      cleanToolOutputsTitle: '清理工具輸出？',
      cleanToolOutputsUpdating: '正在更新預覽…',
      cleanToolOutputsWarning: '這會重寫目前的工作階段歷史並造成一次提示快取未命中。此操作無法復原。',
