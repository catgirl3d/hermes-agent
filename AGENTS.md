...
**Before rebasing or merging Desktop chat/session/multi-pane changes, read [`DESKTOP_SESSION_SWITCH_REBASE_GUIDE.md`](./DESKTOP_SESSION_SWITCH_REBASE_GUIDE.md).** It records the manually verified cold A → B loader behavior, the required hybrid `PRIMARY_SESSION_VIEW` conflict resolution, known rerere hazard, and mandatory regression checks. Do not accept a conflict resolution that replaces snapshot-owned primary identity/messages/run state with `$selectedStoredSessionId`, `$activeSessionId`, `$messages`, or `$busy`.
