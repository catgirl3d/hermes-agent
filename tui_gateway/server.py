...
def _resume_histories(db, session_id: str) -> tuple[list, list]:
    """Load repaired model replay and verbatim display histories in one query."""
...
def _(rid, params: dict) -> dict:
    ...
    if is_truthy_value(params.get("lazy", False)):
        ...
        try:
            ...
            # This resume feeds LIVE REPLAY, so heal durable alternation once
            # here rather than re-firing the pre-request repair every turn.
